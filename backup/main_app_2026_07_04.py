...
def init_db(conn):
    """Crea las tablas necesarias si no existen y asegura las columnas X e Y."""
    c = conn.cursor()
...
def render_carga_datos(conn):
    ...
    if tipo_evento == "Finalizaciones":
        resultado = st.selectbox("Resultado", ["Al arco", "Desviado", "Bloqueado"], key="resultado_rapido")
...
def render_dashboard_general(conn):
    ...
    with c1:
        rivales_disponibles = ["Todos"] + list(df_eventos["rival"].dropna().unique())
        rival_sel = st.selectbox("Filtrar por Rival", rivales_disponibles)
    with c2:
    ...
    if rival_sel != "Todos":
        df_filtrado = df_filtrado[df_filtrado["rival"] == rival_sel]
    ...
    with col2:
        st.metric("Goles/Tiros al Arco", len(df_filtrado[(df_filtrado["tipo_evento"] == "Finalizaciones") & (df_filtrado["resultado"] == "Al arco")]))
    with col3:
...
def render_rendimiento_individual(conn):
    ...
    # Filtramos la base inicialmente por el jugador seleccionado para que los siguientes filtros sean dinámicos
    df_base_jugador = df_eventos[df_eventos["jugador"] == jugador_sel]
    
    with col_f2:
        rivales_disponibles = ["Todos"] + list(df_base_jugador["rival"].dropna().unique())
        rival_sel = st.selectbox("Filtrar por Partido / Rival", rivales_disponibles, key="rend_rival_sel")
    ...
    # Aplicamos los filtros cruzados al DataFrame final del jugador
    df_jugador_filtrado = df_base_jugador.copy()
    if rival_sel != "Todos":
        df_jugador_filtrado = df_jugador_filtrado[df_jugador_filtrado["rival"] == rival_sel]
    if tiempo_sel != "Todos":
    ...
    with m2:
        goles_tiros = len(df_jugador_filtrado[(df_jugador_filtrado["tipo_evento"] == "Finalizaciones") & (df_jugador_filtrado["resultado"] == "Al arco")])
    ...
    with col_mapa:
        ...
        # Reutilizamos la función de análisis robusta con los datos filtrados del jugador
    ...
    with col_tabla:
        ...
        if not df_jugador_filtrado.empty:
            # Mostramos las columnas más relevantes de forma limpia
